...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeletarNotificacao`(IN _usuario VARCHAR(32), IN _notificacao VARCHAR(32)) MODIFIES SQL DATA
BEGIN
    ...
    IF ((SELECT COUNT(*) FROM sacoi.notificacoes WHERE (notificacao = _notificacao)) > 0) THEN
        IF ((SELECT usuario FROM sacoi.notificacoes WHERE (notificacao = _notificacao)) = _usuario) THEN
            DELETE FROM sacoi.notificacoes WHERE notificacao = _notificacao;
